...
>(({ className, children, position = "popper", sideOffset = 6, collisionPadding = 12, ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-[500] max-h-96 min-w-[8rem] overflow-hidden rounded-md border border-border bg-white text-slate-900 shadow-xl dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100",
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    className={cn(
      ...
      "text-slate-800 focus:bg-violet-50 focus:text-violet-900 dark:text-slate-100 dark:focus:bg-violet-950/70 dark:focus:text-white",
      className,
    )}
    {...props}
  >
