...
export function LawsTabPanel({ lawsDraft, onChange, onSave, saving }: Props) {
  return (
    <div className="space-y-5">
      <div className="overflow-hidden rounded-2xl border border-violet-200/90 bg-gradient-to-l from-violet-600 via-violet-700 to-indigo-800 p-5 text-white shadow-lg shadow-violet-500/20 dark:border-violet-500/40">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1 space-y-1">
            <p className="font-display text-[10px] tracking-[0.28em] text-violet-200/90">معاينة للمتقدم</p>
          ...
          <div className="grid gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                className="rounded-xl border-slate-200 bg-slate-50/50 text-right focus-visible:ring-violet-400 dark:border-slate-600 dark:bg-slate-950"
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Button
            ...
            size="sm"
            variant="outline"
            className="gap-1.5 rounded-full border-violet-200 bg-violet-50 text-violet-800 hover:bg-violet-100 dark:border-violet-700 dark:bg-violet-950/40 dark:text-violet-200"
        ...
        <div className="space-y-3">
          {lawsDraft.rules.map((rule, idx) => (
            <div
              key={idx}
              className="group relative overflow-hidden rounded-2xl border border-violet-100 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-slate-600 dark:bg-slate-900/90"
            >
              ...
              <div className="flex gap-3 p-4 pr-5">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-violet-100 font-display text-sm font-bold text-violet-800 dark:bg-violet-950/60 dark:text-violet-200">
                  {idx + 1}
                </span>
                <div className="min-w-0 flex-1 space-y-2">
                  <Textarea
                    ...
                    className="min-h-[80px] resize-y rounded-xl border-slate-200 bg-slate-50/40 text-right text-sm leading-relaxed focus-visible:ring-violet-400 dark:border-slate-600 dark:bg-slate-950"
                    placeholder="اكتب نص البند أو القاعدة..."
                  />
                  {rule.trim() ? (
                    <p className="rounded-lg bg-violet-50/80 px-3 py-2 text-xs leading-relaxed text-slate-700 dark:bg-violet-950/30 dark:text-slate-300">
                      <span className="font-semibold text-violet-700 dark:text-violet-300">معاينة: </span>
                      {rule.trim()}
                    </p>
                  ) : null}
                </div>
