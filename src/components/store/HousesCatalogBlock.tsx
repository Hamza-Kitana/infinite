  ...
  return (
    <div
      className={cn(
        ...
        "bg-gradient-to-br from-emerald-400 via-violet-500 to-cyan-400 bg-[length:300%_300%] animate-vip-gradient-shift motion-reduce:animate-none",
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-0 z-[5] space-y-1.5 px-5 pb-6 pt-24 text-right sm:px-7 sm:pb-8 sm:pt-28">
          <p className="flex items-center justify-end gap-1 text-[11px] font-medium text-white/85">
            <span>{item.district}</span>
            <MapPin className="h-3 w-3" />
          </p>
          <h2 className="font-display text-xl font-bold leading-snug tracking-tight text-white drop-shadow-[0_3px_14px_rgba(0,0,0,0.9)] sm:text-2xl lg:text-[1.55rem]">
            {item.name}
          </h2>
          <div className="flex flex-wrap items-center justify-end gap-3 text-[11px] text-white/85 sm:text-xs">
      ...
      >
        ...
        <div className="flex min-h-[52px] shrink-0 flex-col gap-2 border-t border-primary/15 bg-muted/25 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-start sm:gap-6 sm:px-4 sm:py-2.5">
          <div className="flex w-full justify-start sm:w-auto sm:shrink-0">
            ...
            >
              <Store className="h-4 w-4" /> الاستفسار عن العقار
            </Link>
          </div>
