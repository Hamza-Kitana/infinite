...
type GangStatus = "taken" | "available";

type GangCard = {
  id: string;
  /** الاسم المعروض بالعربية */
  name: string;
  /** الاسم الإنجليزي تحت العنوان */
  nameEn?: string;
  specialty: string;
  location: string;
  description: string;
  /** معرف الفيديو (11 حرفاً) أو رابط يوتيوب كامل — استبدله بالمحتوى الرسمي للعصابة */
  youtubeVideo: string;
  logoImage: string;
  status: GangStatus;
  /** لون تعريف العصابة (HEX) */
  brandColor: string;
  /** نقاط وصف سريعة — هوية العصابة */
  profilePoints: string[];
  /** إذا كانت مأخوذة */
  leaderName?: string;
};

/** فيديوهات يطابق طابع كل عصابة — يمكن استبدالها بروابطكم الرسمية */
const gangs: GangCard[] = [
  {
    id: "yakuza",
    name: "الياكوزا",
    nameEn: "Yakuza",
    ...
    /** سينمائي GTA — طابع ياكوزا / شرق آسيوي */
  ...
  },
  {
    ...
    /** ليل طوكيو / نيون — طابع مدينة مضيئة */
  ...
  },
  {
    ...
    /** تريلر سينمائي — نادي دراجات Lost MC */
